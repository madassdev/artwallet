...
function DataSales(props) {
    useEffect(() => {
        props.fetchAdminTransactions(1, "data");
    }, []);

    const getData = (data) => {
        // console.log(data)
        props.fetchAdminTransactions(data.page, "data");
    };
    return (
        <div>
            {props.isLoading ? (
                    ...
                    <tbody className="tb-odr-body">
                        {props.transactions?.data?.map((t) => (
                            <tr className="tb-odr-item" key={t.id}>
                                ...
                                <p className="tb-odr-id font-bold text-purple-500 uppercase">
...
export default connect(mapState, mapDispatch)(DataSales);
